...
A Comparative Analysis of Naive Bayes, k-Nearest Neighbors, Decision Tree, Random Forest, Gradient Boosting, XGBoost and LightGBM
}
...
\begin{abstract}
At these days, many companies use campaigns in order to make their increase their sales and thus their profits. In order to not waste any resources, predicting the right customer who is going to respond to the campaign is an important part of this process.
But these predictions are not always easy to make because there are many customers and many features that can be used to predict the customer response. In addition to that there is a remarkable class imbalance because only a small percentage of customers respond to the campaign.
...
To overcome the class imbalance, we used class weighting. Evaluation prioritized the F1-Score and Precision-Recall AUC over standard accuracy.
Experimental results demonstrate that ensemble methods, specifically LightGBM combined with engineered metrics, gave the highest predictive performance, achieving an F1-Score of 0.62.
...
\begin{IEEEkeywords}
Data Mining, Customer Response Prediction, LightGBM, SHAP, CRISP-DM.
\end{IEEEkeywords}
...
By doing so we can optimize our budget by not wasting unnecessary money on campaigns and market our products better.
...
Section VI discusses the findings and interprets the models’ business decisions.
Finally, Section VII concludes the paper and mentions about future work.

The dataset was obtained from the Customer Personality Analysis dataset available on Kaggle: https://www.kaggle.com/datasets/imakash3011/customer-personality-analysis.
...
Their ability to model non-linear decision boundaries makes them suitable for many real-world classification problems because mostly real life problems are non-linear.
But, using an only decision tree can be deceiving because it can overfit. especially when the data is noisy or when we dont use pruning.So we need to be careful using them. Despite the overfitting risk, their interpretability valuable.
...
K-NN can be very effective when it is used with appropriate preprocessing and distance metrics.It is important to select the best k value because K-NN is very sensitive to it. Small k value leads to overfitting and large k value to underfitting.
...
We can solve this problem using two solutions. first one is using class weighting and the second one is synthetic sampling like SMOTE. These two solutions prevent the model mainly focusing one one class which is the negative one.\cite{class_imbalance_study}.

Studies that have been done before mainly focused on performance of the models predictions.
In this study, we used different preprocessing strategies, feature engineering techniques, feature selection methods, and multiple classification algorithms to see how it affects the models predictive performance. This part is less common in the literature.
...
In this study we used Customer Personality Analysis dataset from kaggle repository. It is selected because ıt has importantan details about customers which is important for response classification, such as demographic details or previous responses to the campaigns.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.7\columnwidth]{figures/target_distribution.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/financial_metrics.png}}
...
The analysis in this study is for academic purposes. 
\section{Methodology}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/correlation_heatmap.png}}
...
To ensure each algorithm reached its maximum predictive potential, hyperparameter optimization was selectively conducted using \texttt{GridSearchCV} for highly sensitive algorithms, such as k-NN. 
...
As presented in Table \ref{tab:model_results}, ensemble methods were better at classifying the minority class then the baseline models and have a higher positive-class F1-Score. 
The data has class imbalance and because of that, we prioritized the F1-Score and Balanced Accuracy over raw Accuracy. 
LightGBM achieved the highest overall performance in F1-score, and better at correctly identifying positive responders without increasing false positives. 
...
The ROC-AUC score for the Gradient Boosting model was notably higher than its competitors, indicating a strong capacity to distinguish between responders and non-responders. 
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/evaluation_curves.png}}
...
We have found that newly generated engineered features contribute to prediciton ppower better that the other features after analyzing the LightGBM model's feature importance. 
...
Using the SHAP (SHapley Additive exPlanations) framework, we were able to interpret the LightGBM model's decisions. Figure \ref{fig:shap_summary} visualizes the feature importance and the directional impact of each feature on the model's output.



\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/shap_summary.png}}
\caption{SHAP summary plot illustrating the impact of various features on the LightGBM model's predictions.}
...
Even though we used class weighting to deal with the class imbalance, it remained a challenge for the model performance. This showed that the importance of evenly distributed data set. 
...
By following the CRISP-DM methodology, this study showed that the combination of domain-driven feature engineering and the LightGBM classifier achieved better predictive performance than the baseline models. 
...
Furthermore, the usage  of SHAP values transformed the complex predictive model into a more transparent and interpretable decision-support tool. 
...
\begin{table}[htbp]
	...
	\begin{tabular}{|p{0.75\linewidth}|c|}
		\hline
		\textbf{Project Stage} & \textbf{Level} \\
		\hline
		Conceptualization and idea generation & 1 \\
		\hline
		Literature search and synthesis & 3 \\
		\hline
		Methodology, coding, and analysis & 4 \\
		\hline
		Writing, editing, and LaTeX formatting & 3 \\
		\hline
		Interpretation, discussion, and conclusion & 2 \\
		\hline
	\end{tabular}
\end{table}
...
Full source code and Jupyter notebooks are available within the submitted project folder and on GitHub at \url{https://github.com/saadetcansu/CustomerSegmentationProject}. The main execution pipeline is contained in \texttt{BIL476\_Project.ipynb}.
